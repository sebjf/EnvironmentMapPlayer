...
public:
  ...
  {
    if(first > last) throw std::range_error("buffer");
    size_t n = last - first;
    if(n > available()) throw std::out_of_range("buffer");
  ...
  {
    if(first < _start || last > _end_of_storage) throw std::out_of_range("buffer");
